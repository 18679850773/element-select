...
ElementSelect.prototype.handleMode = function (start, end) {
  ...
  if (
    this.config.startMode != "handle" &&
    types.includes(this.config.startMode)
  ) {
    this.keydownbind = function (event) {
      if (
        ((this.config.startMode == "ctrl" && event.key == "Control") ||
...
ElementSelect.prototype.initConfig = function (config) {
  ...
  if (_config.el.constructor == String) {
    ...
    _config.el = _config.el
      .split(",")
      .reduce(
        (a, b) => (a[b.trim()] = document.querySelectorAll(b.trim())) && a,
        Object.create(null)
      ); // .map(e => document.querySelectorAll(e))
    if (!Object.values(_config.el).reduce((a, b) => a + b.length, 0))
      throw "config.el找不到对应的标签";
...
ElementSelect.prototype.createBox = function () {
  ...
  selectBox.style =
    "border:1px solid #409eff;background-color:rgba(255, 255, 255,0.1);" +
...
ElementSelect.prototype._select = function (data) {
  ...
  if (this.config.dataset) {
    const dataType = {
      ...
      Array: (e, f) => {
        dataset[f].push(
          this.config.dataset.reduce(
            (a, b) => (e.dataset[b] ? (a[b] = e.dataset[b]) && a : a),
            Object.create(null)
          )
        );
      },
    ...
    if (dataType[this.config.dataset.constructor.name]) {
      Object.keys(data).forEach((f) => {
        dataset[f] = [];
        data[f].forEach((e) =>
          dataType[this.config.dataset.constructor.name](e, f)
        );
      });
    } else {
...
ElementSelect.prototype.getFilterDoms = function (locator) {
  return locator
    .split(",")
    .reduce((a, b) => (a[b.trim()] = []) && a, Object.create(null));
};
...
ElementSelect.prototype.computeSelectElement = function (scope) {
  ...
  Object.keys(filterDoms).forEach((e) => {
    this.config.el[e].forEach((f) => {
      ...
      if (
        itemOffset.bottom > scope.top &&
        itemOffset.top < scope.bottom &&
        !(itemOffset.right < scope.left || itemOffset.left > scope.right)
      ) {
        filterDoms[e].push(f);
      }
    });
